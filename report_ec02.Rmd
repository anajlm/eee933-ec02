...
Como resultado do teste, obteve-se uma estatística F = 34,66 e um p-valor < 2e-16. 
...
```
```{r fitmodel,results='hold'}
fligner.test(≈ ~ Acoes, data = retornos_melt)
...
```
Não acusa homoscedasticidade;
```{r fitmodel,results='hold'}

plot(x = model$fitted.values, y = model$residuals, xlab = c("Ações"), ylab="Valores",xaxt='n')
```
Caso sorteemos uma amostra aletória do conjunto de dados, não conseguimos afirmar com certeza de qual dos grupos ela foi retirada pela proximidade da variância entre eles. Logo, apesar do teste de Fligner-Killeen não acusar homoscedasticidade, podemos afirmar que essa violação é moderada posto que as observações dos grupos são balanceadas.
...
n<-35
power.anova.test(groups = 5, between.var = vartau,
